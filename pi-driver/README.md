...
3. Explicitly set the Codex channel management parameter `execution_driver` to `pi-experimental`. Blank or `cpa` continues to use CPA. Pi currently admits native HTTP Responses, converted OpenAI Chat/Anthropic Messages/Gemini chat, and local token-count routes covered by the independent conversion contract. WebSocket, search and image operations remain explicitly rejected; unsupported routes fail closed instead of falling back.
...
Credentials are supplied per request. Account ID must exactly equal `https://api.openai.com/auth.chatgpt_account_id` decoded from the JWT's Base64URL payload (optional trailing padding is accepted). Pi 0.85.1's internal `atob` decoder receives a private Base64-normalized parser copy; the per-request fetch hook restores the **original, byte-exact signed token** in network Authorization. The parser copy is never sent upstream. This is claim consistency validation, **not JWT signature verification**; upstream authenticates the token. There is no refresh/login/credential persistence.
...
- Client header overrides are all rejected, including authorization/account/host overrides. Successful responses carry a fixed typed quota allowlist and bounded numeric `Retry-After`, alongside content-type; request IDs, cookies and private turn state are omitted. Quota windows cover primary/secondary and the known Bengalfox namespace, with explicit active-limit attribution. Unknown active limits/namespaces suppress ambiguous generic windows instead of silently attributing them to the account. Error frames may include `retry_after_seconds` (an integer from 0 through 31,622,400); canonical HTTP dates are normalized to delay seconds. Both Node and Go validate these fields. Metadata is scheduling evidence, not authorization to replay an ambiguous attempt.
- Proxy URLs are selected per attempt and injected into Pi's private fetch hook; the sidecar never reads process-global proxy variables for an explicitly selected request. Direct, HTTP, HTTPS and SOCKS5 proxies are validated, bounded and single-attempt. SOCKS5 uses remote DNS (`socks5h` internally), preserves proxy authentication only in the handshake, and never forwards it to the origin. Failed proxies do not fall back to direct. Redirects are never followed; response/error bodies on non-2xx are cancelled without reading or exposing them.
...
This branch does not provide complete Codex/CPA parity: WebSocket/stateful continuation, Codex search, image generation/editing, OAuth refresh reaction to Pi upstream failures, arbitrary header forwarding, incomplete-response success, account scheduling, file/media lifecycle APIs, tools execution, durable sessions, and other providers/operations remain unsupported or retain GPT-Load's existing non-Pi implementation. Live provider compatibility and production throughput have not been verified. Installation uses `--ignore-scripts`; the lockfile pins transitives, but is not a supply-chain audit.
